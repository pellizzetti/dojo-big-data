graph.var.mod = function (x, axes = c(1, 2), xlim = NULL, ylim = NULL, col.sup = "blue", 
    col.var = "black", draw = "all", label = draw, lim.cos2.var = 0.1, 
    cex = 1, cex.lab = 1, cex.axis = 1, title = NULL, new.plot = FALSE, ...) 
{
    if ((!inherits(x, "MFA")) & (!inherits(x, "HMFA")) & (!inherits(x, 
        "MCA")) & (!inherits(x, "PCA"))) 
    ...
    if (!is.null(coord.quanti)) 
        draw.quanti = (lab.quanti <- rep(FALSE, nrow(coord.quanti)))
    if ("all" %in% label) {
        lab.var <- rep(TRUE, nrow(coord.var))
        if (!is.null(coord.quanti)) 
            lab.quanti <- rep(TRUE, nrow(coord.quanti))
    }
    else {
        if ("var" %in% label) 
            lab.var = rep(TRUE, nrow(coord.var))
        else for (j in 1:nrow(coord.var)) if (rownames(coord.var)[j] %in% 
            label) 
            lab.var[j] = TRUE
        if (!is.null(coord.quanti)) {
            if ("quanti.sup" %in% label) 
                lab.quanti <- rep(TRUE, nrow(coord.quanti))
            else for (j in 1:nrow(coord.quanti)) if (rownames(coord.quanti)[j] %in% 
                label) 
                lab.quanti[j] = TRUE
        }
    }
    if ("all" %in% draw) {
        ...
        if (!is.null(coord.quanti)) 
            draw.quanti <- rep(TRUE, nrow(coord.quanti))
    }
    else {
        if ("var" %in% draw) 
            draw.var = rep(TRUE, nrow(coord.var))
    ...
    if (scale.unit) {
        plot(0, 0, xlab = lab.x, ylab = lab.y, xlim = xlim, ylim = ylim, 
            col = "white", asp = 1, cex = cex, cex.lab = cex.lab, cex.axis = cex.lab, main = titre)
        ...
        plot(0, 0, main = titre, xlab = lab.x, ylab = lab.y, 
            ...
            cex = cex, cex.axis = cex.lab)
        abline(v = 0, lty = 2)
        abline(h = 0, lty = 2)
    }
    col.var <- rep(col.var, length = nrow(coord.var))
    for (v in 1:nrow(coord.var)) {
        if (sum(var.cos2[v, ], na.rm = TRUE) < lim.cos2.var) 
            draw.var[v] = FALSE
        if (draw.var[v]) {
            arrows(0, 0, coord.var[v, 1], coord.var[v, 2], length = 0.1, 
                angle = 15, code = 2, col = col.var[v])
            if (lab.var[v]) {
                ...
                text(coord.var[v, 1], y = coord.var[v, 2], labels = rownames(coord.var)[v], 
                  pos = pos, cex = cex, cex.axis = cex.lab, col = col.var[v])
            }
        }
    }
    if (!is.null(coord.quanti)) {
        ...
        for (q in 1:nrow(coord.quanti)) {
            if (sum(quanti.cos2[q, ], na.rm = TRUE) < lim.cos2.var) 
                draw.quanti[q] = FALSE
            if (draw.quanti[q]) {
                ...
                if (lab.quanti[q]) {
                  ...
                  text(coord.quanti[q, 1], y = coord.quanti[q, 
                    2], labels = rownames(coord.quanti)[q], pos = pos, 
                    col = col.sup[q])
                }
            }
        }
